...
export function QuickNavDrawer({
  ...
  const sectionIcon = (id: string) => {
    if (id === 'gioi-thieu') return <GraduationCap className="w-4 h-4 text-blue-500" />;
    if (id === 'du-an') return <FileText className="w-4 h-4 text-blue-500" />;
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700">
          <div className="flex items-center gap-2">
            ...
            <span className="font-extrabold text-blue-950 dark:text-blue-300 text-sm">Điều hướng nhanh</span>
          </div>
          <Button
            type="button"
            onClick={onClose}
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold text-left cursor-pointer ${
                    currentSection === link.id
                      ? 'bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-800'
            ...
            <section>
              ...
              <div className="flex flex-col gap-0.5 max-h-56 overflow-y-auto custom-scrollbar pr-1">
                {stepTexts.map((text, idx) => (
                  ...
                  >
                    <span className="text-blue-500 font-black shrink-0 w-5 text-right leading-tight mt-px">{idx + 1}.</span>
