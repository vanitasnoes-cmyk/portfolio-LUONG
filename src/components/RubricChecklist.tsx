...
function RubricSectionBlock({
  ...
  return (
    <div className="border border-blue-100/50 dark:border-blue-950/50 rounded-xl bg-white/70 dark:bg-slate-900/60 overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-start gap-2 p-3 sm:p-3.5 text-left hover:bg-blue-50/40 dark:hover:bg-slate-800/40 transition-colors cursor-pointer"
        aria-expanded={open}
      >
        <span className="w-7 h-7 rounded-lg bg-blue-600 dark:bg-blue-700 text-white text-[11px] font-black flex items-center justify-center shrink-0">
          {sectionNumber}
        </span>
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs sm:text-sm font-extrabold text-blue-950 dark:text-blue-200 leading-snug">
              {section.title}
            </span>
          ...
          <p className="text-[10px] font-bold text-blue-500 dark:text-blue-400 mt-1">
      ...
      {open && (
        <ul className="px-3 pb-3 space-y-1.5 border-t border-blue-50 dark:border-slate-800/80">
            ...
            <li key={item.id}>
              <label className="flex items-start gap-3 p-2.5 rounded-lg hover:bg-slate-50/80 dark:hover:bg-slate-800/40 cursor-pointer transition-colors">
                <input
                  ...
                  className="mt-0.5 w-4 h-4 rounded border-blue-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 cursor-pointer"
                />
...
export function RubricChecklist(props: RubricChecklistProps) {
  ...
  return (
    <div
      className="rubric-checklist glass-panel rounded-2xl border border-blue-100/40 dark:border-blue-900/30 p-4 sm:p-5 space-y-3 print:border print:border-slate-300"
      data-print-section="rubric"
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          <ClipboardCheck className="w-5 h-5 text-blue-600 dark:text-blue-400 shrink-0" aria-hidden />
          <div>
        ...
        <span className="no-print text-[9px] font-bold text-blue-600 dark:text-blue-300 bg-white/80 dark:bg-slate-850 px-2 py-1 rounded-lg border border-blue-100 dark:border-slate-700">
      ...
      <p className="text-[10px] text-slate-400 dark:text-slate-500 italic leading-relaxed border-t border-blue-100/40 dark:border-slate-800/80 pt-2 print:text-slate-600">
